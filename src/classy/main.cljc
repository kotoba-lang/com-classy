(ns classy.main
  "Kotodama WASM entrypoint for the Classy clean-room actor (L4) — Clojure port.
  ...
  facts (`classy.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "classy")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"classy.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "classy-compat" :tier tier :entities entities} 200])
...
(defn start! [] :classy-compat/ready)
